...
export class FixedRangeComponent implements OnInit {
  ...
  toggleFilter() {
    if (this.isFirstClick || !this.isApplied) {
      this.filterData();
    } else {
      // Refresh logic goes here
      this.clearFilter();
      console.log("caledeedd")
    }
  ...
  filterData() {
    ...
    // Calculate start date as 7 days ago from the current date
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 7);
    const startDateString = startDate.toISOString();
    const endDateString = endDate.toISOString();
    // Send start date and end date for the past 7 days
    this.rangeService.updateSelectedTimeRange(startDateString, endDateString);
    this.fetchData();
  ...
  clearFilter() {
    this.rangeService.updateSelectedTimeRange('', ''); // Set start and end dates to null
    this.fetchData(); // Optionally, fetch data or perform any necessary actions after clearing the filter
    this.isApplied = false; // Reset the applied state
  }
