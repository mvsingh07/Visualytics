  ...
  sendFilePaths(files: File[]) {
    ...
    if (data.status === 200) {
      ...
      if (data.notificationCount > 0) {
        this.http.post('http://192.168.1.131:8021/notifications', { notificationCount: data.notificationCount })
        .subscribe(
            response => {
                console.log('Notification count successfully posted:', response);
            },
            error => {
                console.error('Error posting notification count:', error);
            }
        );
      }
    } else {
